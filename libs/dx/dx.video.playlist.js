/**
* jquery.responsivePlaylist.js
* VERSION 1.1
* By
* Parker W Young
* www.parkerwyoung.com
*
* Joe Lessard
* https://github.com/jlessard
*
* This plugin was a major adaptation of a plugin written by Dan Drayne, his details and the details of the original plugin are found below.
*
*/
...
(function ( $, window, document, undefined ) {
	...
	function getYoutubeVideos(playlistID, cb){
		$.ajax({
			url: 'https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=4&playlistId='+playlistID+'&key=AIzaSyCIlwa-7d7qpKS0Nj5vhI7tb-0minC-qZ8',
			dataType: 'jsonp',
			success: function(result){
	...
	Plugin.prototype = {
				...
				var createListElements = function(){
					...
					$(self.element).find('li').each(function(index) {
						...
						listItem.find('a:first').each(function() {
							...
							thumbHtml = '<span class="rp_thumbnail" style="background-image:url('+thumbUrl+')"></span><p class="rp_title"></p><p class="rp_author"></p>';
			...
			getEmbedCode: function(options, id) {
				if(!isYouTube(id)){ //vimeo
					...
					html += '?';
					html += (options.autoPlayOnLoad) ? 'autoplay=1' : 'autoplay=0';
					...
					html += '&api=1&player_id=vimeoiframe" ';
			...
			handleVideoClick: function(link, options) {
				...
				if(self.options.autoPlayOnLoad && self.isFirst){
					self.autoPlayVideo(link);
				} else if(self.options.autoPlay && !self.isFirst) {
					self.autoPlayVideo(link);
				}
				self.isFirst = false;
			...
			autoPlayVideo: function(link){
				var self = this;

				if(isYouTube(link.data('yt-id'))){

					var onYoutubePlayerStateChange = function(event){
						if(event.data === 0){
							//trigger click on next video
							self.handleClick($('.rp_currentVideo').next('li').children('a'), self.options);
						}
					};
	...
	$(document).ready(function(){
		...
		//$("ul#rp_playlist").append("<div class="loading"></div>")
        ...
        $("ul#rp_playlist").responsiveplaylist({
            ...
            youtube: {
                ...
                theme: 'dark', // 'light' = standard theme, 'dark' = dark theme
                color: 'red', // 'red' = red progress bar, 'white' = white progress bar
            ...
            // youtubeUsername: 'username',
            // vimeoUsername: 'username',
            youtubePlaylist: 'PLt97A5PZovrtRiIPTqZ2zEEycYRgfdpS8',
            // vimeoAlbum: 'XXXXXXX',
            secure: 'auto' //false|true|'auto'
        });
		

		$("#rp_plugin").mousemove(function(){showUI();});
		$("#rp_plugin").tap(function(){showUI();});
		...
		function showUI(){
			...
			$("#rp_plugin").addClass('showingUI');
			$("header").addClass('active');
		}

		function hideUI(){
			$("#rp_plugin").removeClass('showingUI');

			if($("#latest").hasClass("onScreen")) $("header").removeClass('active');
		}
